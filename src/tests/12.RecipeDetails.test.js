...
describe('Testa o componente RecipeDetails', () => {
  ...
  it('Testa se a receita detalhada for um drink é renderizada corretamente', async () => {
    ...
    const { findByText } = renderWithRouterProvider(
      <RecipeDetails match={ { params: { recipeId: '15997' }, url: '/drinks/15997' } } />,
      { route: '/drinks/15997' },
    );
  ...
  it(`Testa se ao clicar no botão compartilhar aparece a mensagem
  "Link copied!"`, async () => {
    mockFetch(dataResponseDetails.meal);
  ...
  it('Testa se a receita estiver como done no localstorage o botão some', async () => {
    mockFetch(dataResponseDetails.meal);
    const data = [
      {
        id: '52771',
